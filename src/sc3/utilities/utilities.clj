...
;; create data store
(def global-directory-objects nil)
...
(defn get-directory-objects-into-global-directory-objects-var
  ...
  [directory]
  (alter-var-root #'global-directory-objects
                  (fn [v directory] (.listFiles (io/file directory)))directory)nil)

(defn filter-dirs 
  []
  (filter some? (map #(if (.isDirectory %)(.getPath %)) global-directory-objects)))

(defn filter-files
  []
  (filter some? (map #(if (.isFile %)(.getPath %)) global-directory-objects)))

(defn filter-filesize
  []
  (filter some? (map #(if (.isFile %)(.length %)) global-directory-objects)))
